...
LokiJSGibbonAdapter.prototype._initializeCollection = function (collectionName) {
    ...
    const self = this;
    self.dbCollection[collectionName] = this.db.getCollection(collectionName);
    ...
    this.dbCollection[collectionName].on('warning', (warning) => {
        ...
        self.emit(this.EVENT.WARNING, warning);
    });
    this.dbCollection[collectionName].on('error', (errDoc) => {
        self.logger.error(errDoc);
        self.emit(this.EVENT.ERROR, errDoc);
    });
    this.dbCollection[collectionName].on('close', () => {
        self.emit(this.EVENT.CLOSE);
    });
    this.dbCollection[collectionName].on('loaded', (loaded) => {
        self.emit(this.EVENT.LOADED, loaded);
    });
 ...
 * @callback _findByNameCallback
 * @param {Error} [error=null] - Error is omitted
 * @param {(object|null)} userObject - One instance of a fetched record
 *
 */
 ...
 * @param {_findByNameCallback} callback
 * @private
 */
 ...
 * @param {initializeCallback} callback - Callback when done
 */
...
LokiJSGibbonAdapter.prototype.removePermission = function (permission, callback) {
    try {
        ...
        const gek = this.dbCollection[COLLECTION.PERMISSION].findAndRemove({'$loki': permissionFoundPosition});
    } catch (error) {
        return callback(error);
    }
    callback();
